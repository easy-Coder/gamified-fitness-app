...
class WorkoutLogRepository {
  ...
  Future<List<WorkoutLog>> getUserWorkoutLogs(String userId) async {
    try {
      final result =
          await _client.from('workout_logs').select().eq('user_id', userId);
      return result.map(WorkoutLogMapper.fromMap).toList();
  ...
  Future<void> addUserWorkoutLog(WorkoutLog workoutLog) async {
    try {
      await _client.from('workout_logs').insert(workoutLog.toMap()
        ..remove('log_id')
        ..remove('completed_at'));
    } on PostgrestException catch (error) {
      throw Failure(message: error.message);
